...
(function(root){
  ...
  function validRecord(x){
    return !!x&&typeof x.id==='string'&&/^[A-Za-z0-9_-]{1,100}$/.test(x.id)&&!['__proto__','constructor','prototype'].includes(x.id)&&
      (x.deleted===true||(typeof x.descricao==='string'&&typeof x.categoria==='string'&&typeof x.valor==='number'&&Number.isFinite(x.valor)&&
       typeof x.data==='string'&&/^\d{4}-\d{2}-\d{2}$/.test(x.data)&&(!x.vencimento||typeof x.vencimento==='string')&&
  ...
  function queue(state,before,after){
    ...
    for(const value of after){
      if(!value||!/^[A-Za-z0-9_-]{1,100}$/.test(value.id)||['__proto__','constructor','prototype'].includes(value.id))throw Error('Identificador inválido.');
      ...
      const previous=state.pending[value.id];
  ...
  function acknowledge(state,sent,response){
    ...
    for(const item of sent){
      const id=item.value.id, pending=state.pending[id];
      if(!pending)continue;
      ...
      }else if(response.conflicts[id]) state.conflicts[id]=response.conflicts[id];
    }
